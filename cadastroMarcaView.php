...
<div class="container mt-3">
    ...
    <a class="btn btn-primary" href="cadastroMarca.php">Incluir</a>
...
<?php
  ...
  $count = $conex->select("select count(*) as qnt from marca");
  ...
  $sql = "SELECT * FROM marca LIMIT $offset, $rowsperpage";  
...
<script>
  ...
  function alterar(id) {
    if(id != null) {
      window.location.href = 'cadastroMarca.php?id='+id;
    } else {
      var checks = document.getElementsByClassName('check');
      var checados = [];
      for (var check of checks) {
  ...
  function excluir() {
    ...
    if(checados.length == 0) {
      ...
      window.location.href = 'excluirMarcaService.php?ids='+ids;
    }
  }
</script>
