...
<div class="container mt-3">
    ...
    <a class="btn btn-primary" href="cadastroVeiculo.php">Incluir</a>
...
<?php
  ...
  $count = $conex->select("select count(*) as qnt from veiculo");
  ...
  $sql = "SELECT * FROM veiculo LIMIT $offset, $rowsperpage";  
  ...
  if(empty($dados)){
    ...
    foreach($dados as $dado) {
      echo '<tr>
              ...
              <td onclick="alterar('.$dado['id'].')">'.$dado['placa'].'</td>
              <td onclick="alterar('.$dado['id'].')">'.$dado['marca'].'</td>
            </tr>';
    }
...
<script>
  ...
  function alterar(id) {
    if(id != null) {
      window.location.href = 'cadastroVeiculo.php?id='+id;
    } else {
      var checks = document.getElementsByClassName('check');
      var checados = [];
      for (var check of checks) {
  ...
  function excluir() {
    ...
    if(checados.length == 0) {
      ...
      window.location.href = 'excluirVeiculoService.php?ids='+ids;
    }
  }
</script>
